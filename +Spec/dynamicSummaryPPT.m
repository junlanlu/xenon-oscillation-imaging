function dynamicSummaryPPT(subj,folder,imgName,dates,amp,nmrFit_ppm)

import mlreportgen.ppt.*;
slidesFile = [folder,'/Subject ',subj,' Spectroscopy Summary.pptx'];
...
replace(presentationTitleSlide,'Title 1',['Subject ',subj,10,'Spectroscopy Summaries']);
...
for slideNum = 1:length(amp)
    ...
    oscs = {''; sprintf('%3.1f', amp(slideNum).area*100); sprintf('%3.2f',amp(slideNum).freq); ...
    ...
    if sum(nmrFit_ppm(slideNum).fwhmG(:)) > 0
        fitType = 'V';
        scan_str = scan_str;
        replace(summarySlide,'Title 1',['Subject ',subj,' (Voigt)',' s',num2str(scan_str)]);
        ...
        replace(summarySlide,'Title 1',['Subject ',subj,' s',num2str(scan_str)]);
        rbc_static = {''; sprintf('%3.2f', nmrFit_ppm(slideNum).area(1));
        sprintf('%4.1f',nmrFit_ppm(slideNum).freq(1));
        sprintf('%2.1f',nmrFit_ppm(slideNum).fwhm(1)); '-----';
        sprintf('%3.1f',nmrFit_ppm(slideNum).phase(1));
    ...
    % Add figures
    p1 = mlreportgen.ppt.Picture(fullfile(folder,['dyn',fitType,imgName{slideNum},'.tif']));
    replace(summarySlide,'dynPicture',p1);
    p2 = mlreportgen.ppt.Picture(fullfile(folder,['dyn',fitType,'_oscs_new',imgName{slideNum},'.tif']));
    replace(summarySlide,'oscillationPicture',p2);
    p3 = mlreportgen.ppt.Picture(fullfile(folder,['static',fitType,imgName{slideNum},'.tif']));
    replace(summarySlide,'staticPicture',p3);
    p4 = mlreportgen.ppt.Picture(fullfile(folder,['dyn',fitType,'_error',imgName{slideNum},'.tif']));
